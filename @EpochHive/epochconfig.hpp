...
// Time based
StaticDateTime[] = {0,0,0,8,0}; //{2014,6,10,7,0}; // {0,0,0,8,0} would forces the server to start at 8am each time it is started while allowing the year, month and day to stay real time. Any values left at 0 will result in no change.
timeDifference = -4; //0; // Server uses real time this will allow you to offset just the hour.
timeMultiplier = 1; //4; // Sets a time multiplier for in-game time. The command range is now capped at 0.1 - 120 to avoid performance problems.
