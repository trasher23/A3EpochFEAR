...
private ["_loc","_rad","_objs","_found"]; // Prevents these variables overwriting existing vars from where this was called from
_loc = _this select 0;
_rad = _this select 1;
...
{
    if (isPlayer _x AND (speed _x) < 200) then // Ignore fast moving players
    {
        _isClose = if (((position _x) distance _loc) < _rad) then { true } else { false };
