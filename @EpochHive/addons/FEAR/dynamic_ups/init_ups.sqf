...
private["_cnps","_towns","_previousTowns","_fnc_getRandomTown","_fnc_createTriggers","_detectRange"];

UPS_TrackPatrol = True;
_detectRange = 1000;
...
_fnc_createTriggers = {
	...
	_this setTriggerArea [_detectRange, _detectRange, 0, false];
	...
	_trig_act_stmnt = format ["[%1, %2] execVM ""\FEAR\dynamic_ups\spawn_ups.sqf""", _pos, _triggerIndex];
...
if (isServer) then {
	...
	_numberOfTriggers = 8; // total triggers to create on map
