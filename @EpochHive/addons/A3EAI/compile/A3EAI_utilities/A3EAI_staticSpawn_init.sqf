...
if ((count _positionArray) isEqualTo 0) then {
	...
	if (A3EAI_debugLevel > 1) then {diag_log format ["A3EAI Debug: Spawn trigger %1 is generating spawn positions from nearby buildings.",triggerText _trigger];};
} else {
	{
	...
	if (A3EAI_debugLevel > 1) then {diag_log format ["A3EAI Debug: Spawn trigger %1 is generating spawn positions from provided markers.",triggerText _trigger];};
};

_newTrigger = createTrigger ["EmptyDetector", _triggerPos,false];
