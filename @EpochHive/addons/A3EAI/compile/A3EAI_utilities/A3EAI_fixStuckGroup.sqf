...
if (_isInfantry) then {
	...
	if (_newPosEmpty isEqualTo []) then {
		_newPosEmpty = [_leaderPos,10 + random(25),random(360),0,[0,0],[25,SPACE_FOR_OBJECT]] call A3EAI_SHK_pos;
	};
	...
	if (({isPlayer _x} count (_newPosEmpty nearEntities [[PLAYER_UNITS,"AllVehicles"], PLAYER_DISTANCE_WITH_LOS_ANTISTUCK]) isEqualTo 0) && {((_newPosEmpty nearObjects [PLOTPOLE_OBJECT,PLOTPOLE_RADIUS]) isEqualTo [])}) then {
		...
		if (A3EAI_debugLevel > 1) then {diag_log format ["A3EAI Debug: Unable to relocate stuck group %1 (%2).",_unitGroup,(_unitGroup getVariable ["unitType","unknown"])];};
	};
} else {
	_newPosEmpty = [0,0,0];
	if (_unitType in ["land","ugv"]) then {
		...
		while {_keepLooking} do {
			_newPosEmpty = [(getMarkerPos "A3EAI_centerMarker"),300 + random((getMarkerSize "A3EAI_centerMarker") select 0),random(360),0,[2,750],[25,_vehicleType]] call A3EAI_SHK_pos;
		...
		_newPosEmpty = [_leaderPos,10 + random(25),random(360),0,[1,300],[25,(typeOf _vehicle)]] call A3EAI_SHK_pos;
	};
	...
	if ((_leader distance (_leader findNearestEnemy _vehicle)) > NEAREST_ENEMY_RANGE_ANTISTUCK) then {
		...
		_vehicle setVelocity [0,0,0.25];
		{
			if ((isNull (objectParent _x)) && {(_x distance _vehicle) > 100}) then {
				_newUnitPos = [_vehicle,25,random(360),0,[0,0],[25,DEFAULT_UNIT_CLASSNAME]] call A3EAI_SHK_pos;
