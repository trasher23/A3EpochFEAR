...
if (_unitGroup getVariable ["CanCheckArea",true]) then {
	...
	if !(isNil "_trigger") then {
		_nearNoAggroAreas = nearestLocations [_leaderPos,["A3EAI_NoAggroArea"],1500];
		...
		if (_inNoAggroArea) then {
			...
			if (A3EAI_debugLevel > 1) then {diag_log format ["A3EAI Debug: Group %1 in no-aggro zone. Moving units toward trigger position.",_unitGroup];};
		};
	};
