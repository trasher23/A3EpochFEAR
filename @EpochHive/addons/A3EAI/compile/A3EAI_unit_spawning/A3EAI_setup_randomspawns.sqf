...
for "_i" from 1 to _maxRandomSpawns do {
	...
	while {
		...
		_posCheckFail = (
			...
			{({if ((_trigPos distance2D _x) < (TRIGGER_SIZE_NORMAL_DOUBLED + A3EAI_minRandSpawnDist)) exitWith {1}} count A3EAI_randTriggerArray) > 0} ||				//Not too close to another random spawn.
