if ((typeName _this) isEqualTo "ARRAY") then {
	...
	if (A3EAI_debugLevel > 1) then {diag_log format ["A3EAI Debug: Creating custom spawn area with params %1.",_this];};
	...
	if !(surfaceIsWater _spawnPos) then {
		...
		_trigger = createTrigger ["EmptyDetector",_spawnPos,false];
