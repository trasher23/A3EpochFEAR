...
if (!isNull _building) then {
	...
	if (_vehSlot != -1) then {
		...
		["Building", _vehHiveKey, []] call EPOCH_server_hiveSET;
		...
		['BuildingKilled', format["%1 was killed by %2 at %3", typeOf _building, _killer, getPosATL _building]] call EPOCH_server_hiveLog;
	};
};
