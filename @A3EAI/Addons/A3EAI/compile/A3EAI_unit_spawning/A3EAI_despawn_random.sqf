...
if (_isForceDespawn) then {
	...
	if (A3EAI_debugLevel > 0) then {diag_log format["A3EAI Debug: All units of random AI group spawned by trigger %1 have been killed. Starting force despawn in 30 seconds.",triggerText _trigger];};
	uiSleep 30;
} else {
	if (A3EAI_debugLevel > 0) then {diag_log format["A3EAI Debug: No players remain in %1. Deleting spawned AI in %2 seconds.",triggerText _trigger,A3EAI_despawnRandomSpawnTime];};
	...
	uiSleep A3EAI_despawnRandomSpawnTime;								//Wait some time before deleting units. (amount of time to allow units to exist when the trigger area has no players)
...
if (_canDespawn) then {
	...
	if (A3EAI_debugLevel > 0) then {diag_log format ["A3EAI Debug: Removing expired random trigger at %1.",mapGridPosition _trigger];};
	deleteVehicle _trigger;
	
	true
} else {
	if (A3EAI_debugLevel > 0) then {diag_log format ["A3EAI Debug: A player has entered the trigger area at %1. Cancelling despawn script.",(triggerText _trigger)];}; //Exit script if trigger has been reactivated since A3EAI_despawnRandomSpawnTime seconds has passed.
