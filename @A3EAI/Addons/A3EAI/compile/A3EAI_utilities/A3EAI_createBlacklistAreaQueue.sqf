...
if (A3EAI_customBlacklistQueue isEqualTo []) then {
	...
	_blacklistQueue = [] spawn {
		while {!(A3EAI_customBlacklistQueue isEqualTo [])} do {
			...
			if (A3EAI_debugLevel > 0) then {diag_log format ["A3EAI Debug: Creating blacklist area at %1 (pos: %2) with radius %3.",_blacklistName,_statement select 0,_statement select 1];};
			uiSleep 1;
		};
	};
} else {
