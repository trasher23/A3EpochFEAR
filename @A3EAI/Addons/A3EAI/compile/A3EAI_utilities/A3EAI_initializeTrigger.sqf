...
call {
	if (_mode isEqualTo 0) exitWith {
		...
		if (A3EAI_debugLevel > 1) then {diag_log format["A3EAI Debug: Initialized static spawn at %1 (%2). GroupArray: %3, PatrolDist: %4. unitLevel: %5. %LocationArray %6 positions, MaxUnits %7, SpawnChance %8.",(triggerText _trigger),(getPosATL _trigger),(_this select 2),(_this select 3),(_this select 4),count (_this select 5),(_this select 6),(_this select 7)];};
	};
	if (_mode isEqualTo 1) exitWith {
		//Dynamic spawns
		_trigger setVariable ["spawnType","dynamic"];
		if (A3EAI_debugLevel > 1) then {diag_log format["A3EAI Debug: Initialized dynamic spawn at %1. GroupArray: %2.",triggerText _trigger,(_this select 2)];};
	};
	if (_mode isEqualTo 2) exitWith {
		//Random spawns
		_trigger setVariable ["spawnType","random"];
		if (A3EAI_debugLevel > 1) then {diag_log format["A3EAI Debug: Initialized random spawn at %1. GroupArray: %2.",triggerText _trigger,(_this select 2)];};
	};
